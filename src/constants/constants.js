...
export const _fetchKarkaRestaurants = async () => {
  try {
...
const _createKarkaRestaurantsArray = rawRestaurantsArray => {
  ...
  for (restaurant of rawRestaurantsArray) {
...
const _getKarkaRestaurantName = karkaRestaurant => {
  const indexOfStartOfRestaurantName =
    karkaRestaurant.indexOf('alt="') + 5; /*for 'alt="'*/
  const indexOfEndOfRestaurantName = karkaRestaurant.indexOf(
    '"',
    /*Starts searching from the start of alt*/
...
const _createKarkaRestaurantMenu = karkaRestaurant => {
  ...
  let readyMenu = [];
  for (meal of rawMealsArray) {
    if (meal !== undefined) {
      readyMeal = _createKarkaRestaurantMealObject(meal);
...
export const _fetchUnicaRestaurants = async () => {
  let promiseArray = [];
...
export const _fetchSodexoRestaurants = async () => {
  try {
    const response = await fetch(
...
export const _getFavoriteRestaurants = async () => {
  try {
    const jsonValue = await AsyncStorage.getItem('favorite_restaurants');
...
export const _storeFavoriteRestaurants = async value => {
  try {
    ...
    await AsyncStorage.setItem('favorite_restaurants', jsonValue);
    return true;
  } catch (e) {
    return false;
  }
